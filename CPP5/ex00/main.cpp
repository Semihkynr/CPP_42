...
/*   Updated: 2026/02/22 11:10:48 by skaynar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main()
{
    ...
    catch (std::exception &e) {std::cerr << "Hata: " << e.what() << std::endl;}
    std::cout << "\n--- Test 2: Grade Too High (Geçersiz Başlangıç) ---" << std::endl;
    try {
        Bureaucrat b2("Tanrı", 0);
        std::cout << b2 << std::endl;
    }
    catch (std::exception &e) {std::cerr << "Hata yakalandı: " << e.what() << std::endl;}
    std::cout << "\n--- Test 3: Grade Too Low (Geçersiz Başlangıç) ---" << std::endl;
    try {Bureaucrat b3("Stajyer", 151);}
